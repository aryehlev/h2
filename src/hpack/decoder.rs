//! Arena-based HPACK decoder with zero-copy fast paths.
//!
...
//!
//! **Header values** use `HeaderValue::from_maybe_shared()` for validated
//! zero-copy construction from `Bytes`.
...
impl WriteArena {
    fn new() -> Self {
        WriteArena {
            // Matches the default HPACK dynamic table size (RFC 7541 §6.1);
            // sufficient for most single HEADERS frames without reallocation.
...
enum DecodedHeader {
    ...
    /// Uses pre-built HeaderName constant + zero-copy HeaderValue.
    StaticName {
        static_idx: u8,
        value_offset: u32,
        value_len: u16,
    ...
    /// Uses known-header fast match + zero-copy HeaderValue.
    Arena {
        name_offset: u32,
        name_len: u16,
        value_offset: u32,
...
impl DynTable {
    fn new(max_size: usize) -> Self {
        DynTable {
            // Cap initial allocation at 16 KB to avoid over-allocating for
            // very large max_size values; storage grows as needed.
            storage: Vec::with_capacity(max_size.min(16384)),
            storage_pos: 0,
            // With 4096-byte table and ~96-byte avg entries (incl. 32-byte
            // overhead per RFC 7541 §4.1), ~40-64 entries is typical.
    ...
    fn insert_raw(&mut self, name: &[u8], value: &[u8]) {
        ...
        // RFC 7541 §4.1: "The size of an entry is the sum of its name's
        // length in octets, its value's length in octets, and 32."
        // The 32-byte overhead estimates the entry structure cost.
    ...
    /// Save current state so it can be restored on partial decode (CONTINUATION frames).
    /// Returns (entries_len, storage_pos, size).
    fn snapshot(&self) -> (usize, usize, usize) {
        (self.entries.len(), self.storage_pos, self.size)
    }

    /// Restore state from a snapshot, undoing any insertions made since the snapshot.
    fn restore(&mut self, (entries_len, storage_pos, size): (usize, usize, usize)) {
...
/// RFC 7541 Appendix A: 61 entries (indices 1-61), index 0 unused.
...
fn decode_int_slow(src: &[u8], pos: &mut usize, mut ret: usize) -> Result<usize, DecoderError> {
    // RFC 7541 §5.1: limit to 5 total bytes (1 prefix + 4 continuation)
    // which supports values up to ~2^28, covering all legitimate HPACK values.
...
/// Arena-based HPACK decoder with zero-copy optimizations.
///
/// Static table entries are materialized directly (no arena, no validation).
/// Known header names bypass `HeaderName::from_lowercase()`.
/// Header values use `from_maybe_shared()` for validated zero-copy construction.
pub struct Decoder {
...
impl Decoder {
    pub fn new(size: usize) -> Self {
        Decoder {
            ...
            // Most HTTP/2 frames have <32 headers; avoids reallocation
            // in the common case.
        ...
        F: FnMut(Header),
    {
        ...
        // Snapshot dynamic table state so we can roll back on NeedMore.
        // CONTINUATION frames re-supply the full buffer, so any dyn_table
        // insertions from a partial decode must be undone to avoid duplicates.
        let dyn_snapshot = self.dyn_table.snapshot();

        let mut pos = 0;
        let result = self.decode_header_block(data, &mut pos, &mut can_resize, &mut arena);

        if let Err(DecoderError::NeedMore(_)) = result {
            self.dyn_table.restore(dyn_snapshot);
            return result;
        }
        result?;
    ...
    /// Inner decode loop, separated so the caller can snapshot/restore dyn_table on NeedMore.
    fn decode_header_block(
        ...
        while *pos < data.len() {
            ...
            if byte & 0x80 != 0 {
                *can_resize = false;
                let index = decode_int_fast(data, pos, 7)?;
                let decoded = self.get_indexed(index, arena)?;
                self.decoded_headers.push(decoded);
            } else if byte & 0x40 != 0 {
                *can_resize = false;
                let decoded =
                    self.decode_literal_fast(data, pos, 6, true, arena)?;
                self.decoded_headers.push(decoded);
            } else if byte & 0xE0 == 0x20 {
                if !*can_resize {
                    return Err(DecoderError::InvalidMaxDynamicSize);
                }
                let new_size = decode_int_fast(data, pos, 5)?;
                ...
                *can_resize = false;
                let decoded =
                    self.decode_literal_fast(data, pos, 4, false, arena)?;
                self.decoded_headers.push(decoded);
            }
        }
        Ok(())
...
impl From<http::header::InvalidHeaderValue> for DecoderError {
    fn from(_: http::header::InvalidHeaderValue) -> DecoderError {
        DecoderError::InvalidUtf8
    }
}

impl From<http::header::InvalidHeaderName> for DecoderError {
    fn from(_: http::header::InvalidHeaderName) -> DecoderError {
        DecoderError::InvalidUtf8
    }
}

impl From<http::method::InvalidMethod> for DecoderError {
    fn from(_: http::method::InvalidMethod) -> DecoderError {
        DecoderError::InvalidUtf8
    }
}

impl From<http::status::InvalidStatusCode> for DecoderError {
    fn from(_: http::status::InvalidStatusCode) -> DecoderError {
        DecoderError::InvalidUtf8
    }
}
...
mod tests {
    ...
    #[test]
    fn test_continuation_dyn_table_not_corrupted() {
        // Simulate a CONTINUATION scenario: first decode fails with NeedMore
        // because the header block is truncated, then the full block succeeds.
        // The dynamic table must not contain duplicate entries.
        let mut decoder = Decoder::new(4096);

        // Build a header block with a literal-with-indexing header:
        // 0x40 = literal with indexing, new name
        // name = "foo" (len 3), value = "bar" (len 3)
        let mut full_block = BytesMut::new();
        full_block.extend_from_slice(&[0x40]);
        full_block.extend_from_slice(&[0x03]); // name len
        full_block.extend_from_slice(b"foo");
        full_block.extend_from_slice(&[0x03]); // value len
        full_block.extend_from_slice(b"bar");

        // First attempt: truncated block (cut off value) — should NeedMore
        let mut truncated = full_block.clone();
        truncated.truncate(5); // only has: 0x40, 0x03, "foo", but no value
        let result = decoder.decode(&mut Cursor::new(&mut truncated), |_h| {});
        assert!(
            matches!(result, Err(DecoderError::NeedMore(_))),
            "expected NeedMore, got {:?}",
            result
        );
        // Dynamic table must be empty — snapshot/restore should have undone the partial insert
        assert_eq!(decoder.dyn_table.entries.len(), 0);

        // Second attempt: full block — should succeed
        let mut buf = full_block;
        let mut headers = vec![];
        decoder
            .decode(&mut Cursor::new(&mut buf), |h| headers.push(h))
            .unwrap();
        assert_eq!(headers.len(), 1);
        // Dynamic table should have exactly 1 entry
        assert_eq!(decoder.dyn_table.entries.len(), 1);
    }
}
